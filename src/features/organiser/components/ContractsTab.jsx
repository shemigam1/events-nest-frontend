            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[1, 2].map(k => (
                    <div key={k} style={{
                        ...
                        border: '1px solid var(--border)', height: 80,
    ...
    return (
        <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <div>
                    <h2 className="mp-h2" style={{ margin: 0, color: 'var(--text-1)' }}>Contracts</h2>
            ...
            {showCreate && (
                <ContractModal eventId={eventId} onDismiss={() => setShowCreate(false)} />
            )}
        </>
    );
}
...
function ContractCard({ contract, eventId }) {
    ...
    const busy = fundState.isLoading || activateState.isLoading ||
                 completeState.isLoading || terminateState.isLoading;

    const s = STATUS_STYLE[contract.status] ?? STATUS_STYLE.DRAFT;
    const isDone = contract.status === 'COMPLETED' || contract.status === 'TERMINATED';
    const hasEscrow = ['FUNDED', 'ACTIVE', 'COMPLETED', 'TERMINATED'].includes(contract.status);
            ...
            {expanded && (
                <div style={{ padding: '20px 20px 24px' }}>
                    ...
                    <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', marginBottom: 16 }}>
                        {contract.signedAt && (
                            <TimestampChip label="Signed" value={fmtDate(contract.signedAt)} />
                        )}
                        {contract.fundedAt && (
                            <TimestampChip label="Funded" value={fmtDate(contract.fundedAt)} />
                        )}
                        {contract.activatedAt && (
                            <TimestampChip label="Activated" value={fmtDate(contract.activatedAt)} />
                        )}
                        {contract.completedAt && (
                            <TimestampChip label="Completed" value={fmtDate(contract.completedAt)} />
                        )}
                        {contract.terminatedAt && (
                            <TimestampChip label="Terminated" value={fmtDate(contract.terminatedAt)} />
                        )}
                    </div>
                    ...
                    {!isDone && (
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: err ? 8 : 0 }}>
                            ...
                            {contract.status === 'FUNDED' && (
                                ...
                                >
                                    {activateState.isLoading ? 'Activating…' : 'Activate contract'}
                                </Button>
                            )}
                    ...
                    {err && (
                        <p style={{ fontSize: 13, color: 'var(--error)', margin: '6px 0 0' }}>{err}</p>
                    )}
...
function EscrowPanel({ contractId, contractStatus }) {
    ...
    return (
        <div style={{ marginTop: 20, borderTop: '1px solid var(--border)', paddingTop: 20 }}>
            ...
            {err && (
                <p style={{ fontSize: 13, color: 'var(--error)', margin: '8px 0 0' }}>{err}</p>
            )}
...
function MilestoneRow({ milestone: m, canRelease, busy, onApprove, onRelease }) {
    ...
    return (
        <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {m.description && (
                    <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 2 }}>{m.description}</div>
                )}
            </div>
...
function ContractModal({ eventId, contract, onDismiss }) {
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            setErr(e?.data?.message ?? (isEdit ? 'Failed to update contract' : 'Failed to create contract'));
        }
    }

    return (
        <Modal open onClose={onDismiss} label={isEdit ? 'Edit contract' : 'New contract'} width={520}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {!isEdit && (
                    <div>
                        ...
                        {appsQ.isLoading ? (
                            <p style={{ fontSize: 13, color: 'var(--text-2)', margin: 0 }}>Loading vendors…</p>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 4 }}>
                    ...
                    <Button type="submit" variant="primary" size="md" disabled={!canSubmit || busy}>
                        {busy ? (isEdit ? 'Saving…' : 'Creating…') : (isEdit ? 'Save changes' : 'Create contract')}
                    </Button>
                </div>
            </form>
            </div>
